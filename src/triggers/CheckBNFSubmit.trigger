trigger CheckBNFSubmit on BNF2__c (before insert, before update) 
{ 
    if(Trigger_Control_For_Migration__c.getInstance() != null && !Trigger_Control_For_Migration__c.getInstance().Disable_BNF_Trigger__c) {
        ...
        Map<Id,Account> ClientAccountMap = new Map<Id,Account>();
        Set<Id> revisedBnfToOpportunityIdSet = new Set<Id>();
        Set<Id> BnfToOpportunityIdSet = new Set<Id>();
        //Added by Himanshu Parashar : 12 dec 2012 : for sending mail
        ...
        for(BNF2__c BNF:Trigger.new){
            revAnalystIdsSet.add(BNF.Revenue_Analyst__c);
        }
        ...
        //  END 2012-01-21 Sam Duncan: Comment out email code to allow deployment of list price validation to PROD            
        
        for (BNF2__c BNF:Trigger.new)
        {
            OpportunityId_Map.put(BNF.Opportunity__c,BNF.Opportunity__c);
            if(BNF.Bill_To__c != null && (Trigger.isInsert || (Trigger.isUpdate && (BNF.Bill_To__c !=Trigger.OldMap.get(BNF.Id).Bill_To__c || BNF.Sales_Org_Code__c !=Trigger.OldMap.get(BNF.Id).Sales_Org_Code__c)))){
                BillToAddressId.add(BNF.Bill_To__c);
            } else if(BNF.Client__c != null && BNF.RecordTypeId == MDM_Defines.Manual_Setup_In_SAP_Record_Type_Id && (Trigger.isInsert || (Trigger.isUpdate && BNF.Client__c != Trigger.oldMap.get(BNF.Id).Client__c))) {
                // Set PO required value for eBNF from Account
                ClientAccountIds.add(BNF.Client__c);
            } else if(BNF.Client__c == null && Trigger.isUpdate && BNF.Client__c != Trigger.oldMap.get(BNF.Id).Client__c && BNF.RecordTypeId == MDM_Defines.Manual_Setup_In_SAP_Record_Type_Id) {
                BNF.Is_PO_Required__c = false;
            } else if(BNF.Bill_To__c == null && Trigger.isUpdate && BNF.Bill_To__c != Trigger.oldMap.get(BNF.Id).Bill_To__c && BNF.RecordTypeId == MDM_Defines.SAP_SD_Integrated_Record_Type_Id) {
                BNF.Is_PO_Required__c = false;
            }
            if(Trigger.isInsert && BNF.RecordTypeId == MDM_Defines.SAP_SD_Integrated_Record_Type_Id){
                if(BNF.Addendum__c){
                    if(!(revisedBnfToOpportunityIdSet.contains(BNF.Opportunity__c))){
                        revisedBnfToOpportunityIdSet.add(BNF.Opportunity__c);
                    }
                }else{
                    if(!(BnfToOpportunityIdSet.contains(BNF.Opportunity__c))){
                        BnfToOpportunityIdSet.add(BNF.Opportunity__c);
                    }
                }
            }
        }
        
        List<BNF2__c> bnfToRevisedBnfList = new List<BNF2__c>();
        Map<id,String> oppIdToErrorMsgMap = new Map<id,String>();
        Map<Id,Boolean> opptoRevisedBnfMap = new Map<Id,Boolean>();
        if(revisedBnfToOpportunityIdSet.size() > 0 ){
            bnfToRevisedBnfList = [Select Has_Pricing_Configuration__c,Opportunity__c from BNF2__c where Opportunity__c =:revisedBnfToOpportunityIdSet and Addendum__c = false ];
            For(BNF2__c bnf : bnfToRevisedBnfList){
                opptoRevisedBnfMap.put(bnf.Opportunity__c,bnf.Has_Pricing_Configuration__c);
            }
        }
        if(BnfToOpportunityIdSet.size() > 0 ){
            oppIdToErrorMsgMap = CPQ_QuoteUtilityWO.validateQuoteSyncWithOpportunities(BnfToOpportunityIdSet);
        }
        
        for(BNF2__c BNF:Trigger.new){
            if(BNF.Addendum__c && opptoRevisedBnfMap.containsKey(BNF.Opportunity__c)){
                BNF.Has_Pricing_Configuration__c = opptoRevisedBnfMap.get(BNF.Opportunity__c);
            }else{
                if(oppIdToErrorMsgMap.containsKey(BNF.Opportunity__c) && oppIdToErrorMsgMap.get(BNF.Opportunity__c) == 'Success' && BNF.Addendum__c == false ){
                    BNF.Has_Pricing_Configuration__c = true;
                }
            }
        }

        // Process Is_PO_Required__c value for eBNF from Account
        if(ClientAccountIds.size() > 0) {
            ClientAccountMap = new SLT_Account().selectByAccountId(ClientAccountIds, new Set<String>{'Id','PO_Number__c'}); 
            for (BNF2__c BNF:Trigger.new) {
                if(BNF.Client__c != null && ClientAccountIds.contains(BNF.Client__c)) {
                    BNF.Is_PO_Required__c = ClientAccountMap.get(BNF.Client__c).PO_Number__c;
                }
            }
        }
        ...
        if(BillToAddressId.size() > 0){
            ...
            for (BNF2__c BNF:Trigger.new){
                if(BNF.Bill_To__c != null && BillToAddressId.contains(BNF.Bill_To__c)){
                    if(BNF.RecordTypeId == MDM_Defines.SAP_SD_Integrated_Record_Type_Id && (Trigger.isInsert || (Trigger.isUpdate && BNF.Bill_To__c != Trigger.OldMap.get(BNF.Id).Bill_To__c))) {
                        // Process Is_PO_Required__c value for iBNF from Address
        ...
        // Jeganmohan N, 12/12/2011, MercuryTicket #10524, Check whether the pricing discount approval is pending for the opportunity    
        Map<Id,Opportunity> Opportunity_Map = new Map<Id,Opportunity>([select Id, opportunity_number__c, Actual_Close_Timestamp__c,therapy_area__c,Contract_Start_Date__c,Account.MDMID__c, Contract_End_Date__c,StageName,pse__Primary_Project__c,SAP_Master_Contract__c,Discount_Approval_Status__c,Is_USBU_Opportunity__c from Opportunity where Id in :OpportunityId_Map.keySet()]);
...
Date- 20 Dec, 2012 
Issue-01473
insert PricebookEntry.Product2.Offering_Type__c,List_Price__c in opportunityLineItem for Issue-01473
*/
        ...
        Map<Id,BNF2__c> BNF_Opp_Id_Map = new Map<Id,BNF2__c>();
        ...
        for(BNF2__c b : Trigger.new)
        {
            BNF_Opp_Id_Map.put(b.Opportunity__c,b);
            // Added by Ritesh gupta - ER-3208
            if(b.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SAP_CONTRACT_CONFIRMED') ){
                isBNFSAPConfirmed = true;
            }
            if(Opportunity_Map.containsKey(b.Opportunity__c)){
                b.SAP_PC_Code__c = Opportunity_Map.get(b.Opportunity__c).Account.MDMID__c;
            }
            ...
            AddressId_Set.add(b.Bill_To__c);
            AddressId_Set.add(b.Ship_To__c);
            AddressId_Set.add(b.Cover_Sheet__c);
            AddressId_Set.add(b.X2nd_Copy__c);
            AddressId_Set.add(b.Carbon_Copy__c);
            AddressId_Set.add(b.Sold_To__c);
            
            Address_Map.put(b.X2nd_Copy__c,'2nd Copy');
            Address_Map.put(b.Carbon_Copy__c,'Carbon Copy');
            Address_Map.put(b.Bill_To__c,'Bill To');
            Address_Map.put(b.Ship_To__c,'Ship To');
            Address_Map.put(b.Cover_Sheet__c,'Cover Sheet');
        ...
        if (trigger.isUpdate)
        {
            ...
            for(AggregateResult agg : OLSSchedule_Array)
            {
                ...
                Date StartDate = (BNF_Opp_Id_Map.get(String.valueof(agg.get('OpportunityId'))).Addendum__c ? Date.valueof(agg.get('Product_Start_Date__c')) : Date.valueof(agg.get('Project_Start_Date__c')));
        ...
        List<OpportunityLineItem> oliList1 = [select Id, Nbr_of_Users__c, PO_line_item_number__c, UnitPrice,Budget_Amount__c, Budget_Therapy_Area__c,Billing_Schedule__c, Billing_Schedule__r.Total_Billing_Amount__c, Revised_Price__c,Delivery_Media__c,Billing_Date__c,Opportunity.CEQActiveRelease__c,Revenue_Type__c,PricebookEntry.ProductCode,Delivery_Date__c,Product_End_Date__c,Product_Start_Date__c,Therapy_Area__c,OpportunityId,PricebookEntry.Product2.Offering_Type__c,List_Price__c,Sale_Type__c,Billing_Frequency__c,Proj_Rpt_Frequency__c,PricebookEntry.Product2.Material_Type__c,PricebookEntry.Product2.ProductCode,Wbsrelementcode__c,TotalPrice,PricebookEntry.Product2.Enabled_Sales_Orgs__c,Surcharge_Text__c,Discount_Reason__c,PricebookEntry.Product2.Item_category_group__c, PO_Number__c,Revised_Revenue_Schedule__c,Project_End_Date__c,Discount_Amount_Formula__c,Other_Ship_To_Address__c, Other_Ship_To_Address__r.name, Other_Ship_To_Address__r.SAP_Reference__c, Other_Ship_To_Address__r.Enabled_Sales_Orgs__c,Audit_Subscription_Status__c,  Product_Material_Type__c from OpportunityLineItem where OpportunityId in :Opportunity_Map.keySet()];
        ...
        if(isBNFSAPConfirmed){
            ...
            //query += ', (select Description, Revenue, ScheduleDate from OpportunityLineItemSchedules order by ScheduleDate asc)';
            query += ' FROM OpportunityLineItem where OpportunityId IN (';
            for(Id oppId : Opportunity_Map.keySet()){
                query += '\'' + oppId + '\',';
            }
        ...
        //for (OpportunityLineItem OLI : [select Id, UnitPrice,Budget__c, Budget_Amount__c, Budget_Therapy_Area__c,Billing_Schedule__c, Billing_Schedule__r.Total_Billing_Amount__c, Revised_Price__c,Billing_Date__c,Opportunity.CEQActiveRelease__c,Revenue_Type__c,PricebookEntry.ProductCode,Delivery_Date__c,Product_End_Date__c,Product_Start_Date__c,Therapy_Area__c,OpportunityId,PricebookEntry.Product2.Offering_Type__c,List_Price__c,Sale_Type__c,Billing_Frequency__c,Proj_Rpt_Frequency__c,PricebookEntry.Product2.Material_Type__c,PricebookEntry.Product2.ProductCode,Wbsrelementcode__c,TotalPrice,PricebookEntry.Product2.Enabled_Sales_Orgs__c,Surcharge_Text__c,Discount_Reason__c,PricebookEntry.Product2.Item_category_group__c, PO_Number__c,Revised_Revenue_Schedule__c,Project_End_Date__c,Discount_Amount_Formula__c,Other_Ship_To_Address__c from OpportunityLineItem where OpportunityId in :Opportunity_Map.keySet()])
        for (OpportunityLineItem OLI : oliList)
        {
            if (!OLI_Array_Map.containsKey(OLI.OpportunityId))
            {
                OLI_Array_Map.put(OLI.OpportunityId,new OpportunityLineItem[]{OLI});
            }
            else
            {
                OLI_Array_Map.get(OLI.OpportunityId).add(OLI);
            }
        ...
        //system.assert(false, OLI_Array_Map);
        //Added by Himanshu Parashar : 13 dec 2012 : Map for BNF fields
        Map<Id,BNF2__c> bnfMap = new Map<Id,BNF2__c>([select Revenue_Analyst__r.Is_SAP_Revenue_Analyst__c,Revenue_Analyst__r.RA_Sales_Orgs__c,Revenue_Analyst__r.User__r.TimeZoneSidKey,Revenue_Analyst__r.User__r.Email,Revenue_Analyst__r.User__r.Name,Bill_To__r.Name,Ship_To__r.Name,X2nd_Copy__r.Name,Cover_Sheet__r.Name,Carbon_Copy__r.Name,Bill_To__r.SAP_Reference__c,Ship_To__r.SAP_Reference__c,X2nd_Copy__r.SAP_Reference__c,Carbon_Copy__r.SAP_Reference__c,Cover_Sheet__r.SAP_Reference__c, Bill_To__r.PO_Required__c from BNF2__c where Id In:Trigger.new ]);
        ...
        List<Address__c> Address_List = new List<Address__c>([select Id,Name,SAP_Reference__c,Payment_Terms__c,Enabled_Sales_Orgs__c,Marked_For_Deletion__c from Address__c where Id in :AddressId_Set and Id != null]);
		
            for(Address__c Address : Address_List){
        ...
        for (BNF2__c  cBNF : Trigger.new) 
        {
            ...
            if(Trigger.isUpdate && cBNF.BNF_Status__c == 'Submitted' && cBNF.BNF_Status__c != Trigger.oldMap.get(cBNF.Id).BNF_Status__c){
                ...
                if(String.isBlank(cBNF.AR_Comments__c) && Opportunity_Map.get(cBNF.Opportunity__c).Is_USBU_Opportunity__c  
                    && cBNF.Addendum__c && cBNF.Addendum_Reasons__c == 'Change Order/Amendment' && enableAgreementRepository){
            ...
            if (Trigger.isUpdate && cBNF.BNF_Status__c == 'Submitted'  && cBNF.BNF_Status__c != Trigger.oldMap.get(cBNF.Id).BNF_Status__c && lqOppIdSet.contains(cBNF.Opportunity__c)) {
                cBNF.addError('Please set Quintiles Product amount to zero before submitting the BNF.');
            }
            if(Trigger.isUpdate && cBNF.BNF_Status__c == 'Submitted'  && cBNF.BNF_Status__c != Trigger.oldMap.get(cBNF.Id).BNF_Status__c && cBNF.Revenue_Analyst__c != null && revAnalystRecordsMap.get(cBNF.Revenue_Analyst__c).Disable_RA__c) {
                cBNF.addError('Please select a valid Revenue Analyst.');
            }
            try {
                if(bnfSetting.BNF_Opportunity_Threshold__c != null) {
                    opportunityNumber = Opportunity_Map.get(cBNF.Opportunity__c).opportunity_number__c;
                ...
                && Trigger.oldMap.get(cBNF.Id).BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED'))
            {
                if (cBNF.Rejection_Reasons_Multi__c == null)
                {
                    ...
                    {
                        cBNF.addError('BNF\'s cannot be rejected through this screen.  Please click <a href="/apex/BNF_Approval?id='+ cBNF.id +'">here</a> to Approve/Reject this BNF');
                    }
                }
            }     
            if (Trigger.isUpdate &&
                ...
                BNF_Approval_Extension.CustomApprovalPage == false)
            {
                cBNF.addError('BNF\'s cannot be approved through this screen.  Please click <a href="/apex/BNF_Approval?id='+ cBNF.id +'">here</a> to Approve/Reject this BNF');
            }
            
            //  Ensure that accepted BNF's cannot be re-submitted  
            if (Trigger.isUpdate &&
            ...
            {
                cBNF.addError('This BNF has already been submitted and cannot be re-submitted.');
            } 
            //  Ensure that opportunity is at stage 6a or In-Hand when BNF is submitted
            if (Trigger.isUpdate && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED') && Opportunity_Map.get(cBNF.Opportunity__c).StageName != '7a. Closed Won' && Opportunity_Map.get(cBNF.Opportunity__c).StageName != 'In-Hand')
            {
                cBNF.addError('This opportunity must be at stage "7a. Closed Won" or "In-Hand" before you can submit the BNF.');
            }
            else if (Trigger.isBefore && Trigger.isUpdate && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED') && cBNF.BNF_Status__c != Trigger.oldMap.get(cBNF.ID).BNF_Status__c && cBNF.Revenue_Analyst__c == null)
            {
            ...
            if (cBNF.RecordTypeId != MDM_Defines.SAP_SD_Integrated_Record_Type_Id && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('REJECTED') && Trigger.oldMap.get(cBNF.ID).BNF_Status__c != MDM_Defines.BnfStatus_Map.get('REJECTED'))      
            {
                cBNF.RA_Rejection_Count__c = cBNF.RA_Rejection_Count__c==null ? 1 : cBNF.RA_Rejection_Count__c+1;
            }
            ...
            Boolean iBNF_Enabled = (cBNF.RecordTypeId == MDM_Defines.SAP_SD_Integrated_Record_Type_Id); 
            ...
            List<OpportunityLineItem> OLI_array = OLI_Array_Map.get(cBNF.Opportunity__c);
            
            if(cBNF.RecordTypeId == MDM_Defines.Manual_Setup_In_SAP_Record_Type_Id && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED')){
                BNFProductController ebnfCls = new BNFProductController();
                ebnfCls.seteBNF(cBnf.Id);
                ebnfCls.setOpportunityLineItem(OLI_array);
                pagereference getRetureValue = ebnfCls.save();
            ...
            if(cBNF.RecordTypeId == MDM_Defines.SAP_SD_Integrated_Record_Type_Id && OLI_array!=null && OLI_array.size()>0 &&
               (cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED')) 
               && Trigger.isUpdate && BNFOpptyListLockedGrid.CheckValidation==true)
            {
                BNFOpptyListLockedGrid bnfcls=new BNFOpptyListLockedGrid();
                bnfcls.setOpportunityLineItem(OLI_array);
                bnfcls.setBNF(cBnf); 
                bnfcls.setPORequiredOnBNF(bnfMap.get(cBnf.id).Bill_To__r.PO_Required__c);
                bnfcls.setIsNewBnfFlag();
                if(bnfcls.uiRequiredFieldChck() && !Test.isRunningTest()){
                    cBNF.addError('Please use the "Edit Product" button on the BNF to review and correct all product details.');
                }
                if(bnfcls.ValidateOLI() == false) {
                    if(bnfSetting.Enable_PO_Validation__c && bnfcls.poErrorOccured) {
                        cBNF.addError('PO Number is required in order to submit record for approval.');                 
                    } else {
                        cBNF.addError('Please use the "Edit Product" button on the BNF to review and correct all product details.');
                    }   /*else if(String.isNotBlank(bnfcls.oliTherapyError)){
cBNF.addError(bnfcls.oliTherapyError);       
} */            
                }        
                
                //system.assert(false,'*************** - ' + bnfcls.ValidateOLI());        
            }
...
*/
                    ...
                    if(Trigger.isUpdate)
                    {
                        if(((Trigger.oldMap.get(cBNF.Id).BNF_Status__c!=null && Trigger.oldMap.get(cBNF.Id).BNF_Status__c.equals(MDM_Defines.BnfStatus_Map.get('NEW')) && cBNF.BNF_Status__c.equals(MDM_Defines.BnfStatus_Map.get('SUBMITTED')))||(Trigger.oldMap.get(cBNF.Id).BNF_Status__c.equals('Rejected')&&cBNF.BNF_Status__c.equals(MDM_Defines.BnfStatus_Map.get('SUBMITTED')))) && (!cBNF.Sales_Org_Code__c.contains('US')) && (OLI.PricebookEntry.Product2.Offering_Type__c !='Management Consulting' && OLI.PricebookEntry.Product2.Offering_Type__c !=Label.CES_TOT_Offering_Type && OLI.List_Price__c == null ))
                        ...
                        if(cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED')){
                            if(OLI.Budget_Amount__c != null && OLI.UnitPrice != OLI.Budget_Amount__c){
                                cBNF.addError('Sales price of opportunity line item does not match corresponding budget amount');
                            }
                        }
                    ...
                    if (OLI.PricebookEntry.Product2.Material_Type__c == 'ZREP')
                    {
                        ...
                        /*if(cBNF.RecordTypeId == MDM_Defines.SAP_SD_Integrated_Record_Type_Id && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED') &&
Trigger.oldMap.get(cBNF.Id).BNF_Status__c != MDM_Defines.BnfStatus_Map.get('SUBMITTED') &&
OLI.Billing_Schedule__c==NULL)
{
...
BillingScheduleProductCodes+= OLI.PricebookEntry.Product2.ProductCode;                                            
                        ...
                        if(IsNewBnf) {
                            if(OLI.Revised_Revenue_Schedule__c == null && cBNF.RecordTypeId == MDM_Defines.SAP_SD_Integrated_Record_Type_Id && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED') &&
                               Trigger.oldMap.get(cBNF.Id).BNF_Status__c != MDM_Defines.BnfStatus_Map.get('SUBMITTED') &&
                            ...
                            if(OLI.PricebookEntry.Product2.Material_Type__c == 'ZREP' && (cBNF.RecordTypeId == MDM_Defines.SAP_SD_Integrated_Record_Type_Id) && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED') &&
                               Trigger.oldMap.get(cBNF.Id).BNF_Status__c != MDM_Defines.BnfStatus_Map.get('SUBMITTED') &&
                    ...
                    if(IsNewBnf) {
                        ...
                        if (OLI.PricebookEntry.Product2.Material_Type__c == 'ZREP'|| (OLI.PricebookEntry.Product2.Material_Type__c=='ZPUB' && 
                                                                                      OLI.PricebookEntry.Product2.Item_Category_Group__c=='ZLIC')) {
                                                                                          ...
                                                                                          if(cBNF.RecordTypeId == MDM_Defines.SAP_SD_Integrated_Record_Type_Id && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED') &&
                                                                                             (Trigger.oldMap != null && Trigger.oldMap.get(cBNF.Id).BNF_Status__c != MDM_Defines.BnfStatus_Map.get('SUBMITTED')) &&
                                                                                          ...
                                                                                          {
                                                                                              //system.assert(false, ' OLI.Billing_Schedule__c = ' + OLI.Billing_Schedule__c + ', OLI.Billing_Schedule__r.Total_Billing_Amount__c.setscale(2) = ' + OLI.Billing_Schedule__r.Total_Billing_Amount__c + ', OLI.TotalPrice.setscale(2) = ' + OLI.TotalPrice);
                                                                                              BillingScheduleEmpty = true;
                                                                                              if (BillingScheduleProductCodes != '')
                                                                                              {
                                                                                                  BillingScheduleProductCodes += ', ';
                    ...
                    //Added by Himanshu Parashar : 11 dec : BNF_Status__c Submitted
                    ...
                    //if( bnfSetting.Enable_Material_Validation__c==true && cBNF.BNF_Status__c == 'Submitted' && Trigger.oldMap.get(cBNF.Id).BNF_Status__c != 'Submitted' && (OLI.PricebookEntry.Product2.Enabled_Sales_Orgs__c== null ||(setProductSalesOrgs.size()>0 && OLI.PricebookEntry.Product2.Material_Type__c == 'ZPUB' && (!setProductSalesOrgs.contains(cBNF.Sales_Org_Code__c)))))  
                    if((OLI.TotalPrice!=null && OLI.TotalPrice > 0) && bnfSetting.Enable_Material_Validation__c==true && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED') && Trigger.oldMap.get(cBNF.Id).BNF_Status__c != MDM_Defines.BnfStatus_Map.get('SUBMITTED') && (OLI.PricebookEntry.Product2.Enabled_Sales_Orgs__c== null ||(setProductSalesOrgs.size()>0 && (!setProductSalesOrgs.contains(cBNF.Sales_Org_Code__c)))))
                    ...
                    //Added By Himanshu Parashar : MIS Issue-02000 : Update lineitem if no therapy_area specified
                    if(cBNF.Addendum__c && OLI.Therapy_Area__c == null && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED') && Opportunity_Map.get(cBNF.Opportunity__c).Therapy_Area__c!=null)
                    {
                        
                        OLI.Therapy_Area__c = Opportunity_Map.get(cBNF.Opportunity__c).Therapy_Area__c;
                    ...
                    if(cBNF.RecordTypeId == MDM_Defines.SAP_SD_Integrated_Record_Type_Id && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED') &&
                       Trigger.oldMap.get(cBNF.Id).BNF_Status__c != MDM_Defines.BnfStatus_Map.get('SUBMITTED') && OLI.PricebookEntry.Product2.Material_Type__c == 'ZREP')
                    {
                        if (OLI.Product_End_Date__c > cBNF.Contract_End_Date__c)
                        {
                        ...
                        if (OLI.Product_Start_Date__c < cBNF.Contract_Start_Date__c)
                        {
                ...
                if(IsNewBnf) {            
                    ...
                    {
                        cBNF.addError('BNF cannot be submitted at this time because Product(s): '  + BillingScheduleProductCodes + ' do not have billing schedules equal to sales price. Please use the "Edit Product" button on the BNF to edit these products and create/edit the Billing Schedule. Once complete you can submit the BNF');
                    }
                    
                    if(bnfSetting.Enable_RSchedule_Validation__c && RevenueScheduleEmpty)
                    {
                        cBNF.addError('BNF cannot be submitted at this time because Product(s): '  + RevenueScheduleProductCodes + ' have a invalid Revenue Schedules. Revenue schedule should be between Project Start/End Date. Once complete you can submit the BNF');
                    }
...
Date- 20 Dec, 2012 
Issue-01473
error added, if null and Non-US sales org
*/
            ...
            cBNF.SAP_Contract_Type__c = AdHoc ? 'AD-HOC' : 'CONTRACT';
            //Added by arun 20/08/2019
            ...
            if(Trigger.isUpdate &&  Trigger.oldMap.get(cBNF.Id).Manual_Handling_in_SAP__c != cBNF.Manual_Handling_in_SAP__c && cBNF.IMS_Sales_Org__c == CON_CRM.IMS_SALES_ORG_CH07){
                ...
                Integer countInt = 1;
                while(countInt < 11) {
                    Object revId = cBNF.get('Revenue_Analyst_'+ count +'__c');
                    if(revId != null) {
                        revAnalystIdsSetToCmpr.add((Id)revId);
                    }
                    countInt++;
                    count =  'User_' + String.valueOf(countInt);
                }
                if(revAnalystRecordsMap.get(cBNF.Revenue_Analyst__c).Owner.Type == CON_CRM.QUEUE && revAnalystIdsSetToCmpr.contains(UserInfo.getUserId())){
                    isCurrentUserIsSBSUser = true;
                }
            }
            if(IsNewBnf) {
                if (iBNF_Enabled && (cBNF.Addendum__c || ExcludedLineItems > 0 || MDM_Defines.ManualSalesOrgs.contains(cBNF.IMS_Sales_Org__c)))
                {
                ...
                if (iBNF_Enabled && (cBNF.Addendum__c || ExcludedLineItems > 0 || ZrepLineItems > 0 || MDM_Defines.ManualSalesOrgs.contains(cBNF.IMS_Sales_Org__c)))
                {
                    if(!isCurrentUserIsSBSUser){
                    cBNF.Manual_Handling_in_SAP__c = true;
                    }else{
...
{
cBNF.SAP_Master_Contract__c = Opportunity_Map.get(cBNF.Opportunity__c).SAP_Master_Contract__c;              
}*/
                //Update by : Himanshu : 6 Nov 2013 : Issue-03071
                if (cBNF.SAP_Master_Contract__c == null && Opportunity_Map.get(cBNF.Opportunity__c).SAP_Master_Contract__c != null)
                {
                    cBNF.SAP_Master_Contract__c = Opportunity_Map.get(cBNF.Opportunity__c).SAP_Master_Contract__c;              
                }
                else if (cBNF.SAP_Master_Contract__c == null && cBNF.Prior_Opportunity_SAP_Master_Contract__c != null)
                {
                    cBNF.SAP_Master_Contract__c = cBNF.Prior_Opportunity_SAP_Master_Contract__c;
                }
            }
            ...
            // Jeganmohan N, 12/12/2011, MercuryTicket #10524, Check whether the pricing discount approval is pending for the opportunity
            /*if (Trigger.isUpdate && cBNF.BNF_Status__c == 'Submitted' && cBNF.Has_Pricing_Configuration__c
&& Opportunity_Map.get(cBNF.Opportunity__c).Discount_Approval_Status__c == 'Pending')
{
cBNF.addError('Pricing discount approval is pending for this opportunity. Please get it approved before submitting this BNF for approval.');
}*/              
...
{*/
            ...
            if(sStatus ==MDM_Defines.BnfStatus_Map.get('SUBMITTED'))
            {
                if(ZrepLineItems > 0 && WbsRequired)
                {
            ...
            if(Trigger.isUpdate && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED') && Trigger.oldMap.get(cBNF.Id).BNF_Status__c != cBNF.BNF_Status__c  && cBNF.RecordTypeId == MDM_Defines.SAP_SD_Integrated_Record_Type_Id )
            {
            ...
            /*
            if(cBNF.RecordTypeId == CON_CRM.BNF_RECORD_TYPE_SAP_INTEGRATED && cBNF.Bill_To__c != null && AddressMap.containsKey(cBNF.Bill_To__c) && (Trigger.isInsert || (Trigger.oldMap.get(cBNF.Id).Bill_To__c != cBNF.Bill_To__c) )){
                cBNF.Payment_Terms__c = AddressMap.get(cBNF.Bill_To__c).Payment_Terms__c;   
            }
            
            */
            if(bnfSetting.Enable_Customer_Validation__c==true && cBNF.BNF_Status__c == MDM_Defines.BnfStatus_Map.get('SUBMITTED') && Trigger.oldMap.get(cBNF.Id).BNF_Status__c != MDM_Defines.BnfStatus_Map.get('SUBMITTED'))// && Trigger.oldMap.get(cBNF.id).Sales_Org_Code__c != cBNF.Sales_Org_Code__c )
            {
                // Issue-08506
                ...
                if(mdmUserList.size() > 0) {
                    ...
                    for (Address__c A:Address_List)
                    {
                        if(A.Enabled_Sales_Orgs__c!=null)
                        {
                            ...
                            {      
                                ...
                                if(Address_Map.get(A.Id) == 'Bill To')
                                {
                                    htmlBody +='<tr><td>'+Address_Map.get(A.Id)+': </td><td>'+bnfMap.get(cBNF.Id).Bill_To__r.Name;
                                ...
                                else if(Address_Map.get(A.Id) == 'Ship To')
                                {
                                    htmlBody +='<tr><td>'+Address_Map.get(A.Id)+': </td><td>'+bnfMap.get(cBNF.Id).Ship_To__r.Name;
                                ...
                                else if(Address_Map.get(A.Id) == '2nd Copy')
                                {
                                    htmlBody +='<tr><td>'+Address_Map.get(A.Id)+': </td><td>'+bnfMap.get(cBNF.Id).X2nd_Copy__r.Name;
                                ...
                                else if(Address_Map.get(A.Id) == 'Carbon Copy')
                                {
                                    htmlBody +='<tr><td>'+Address_Map.get(A.Id)+': </td><td>'+bnfMap.get(cBNF.Id).Carbon_Copy__r.Name;
                                ...
                                else if(Address_Map.get(A.Id) == 'Cover Sheet')
                                {
                                    htmlBody +='<tr><td>'+Address_Map.get(A.Id)+': </td><td>'+bnfMap.get(cBNF.Id).Cover_Sheet__r.Name;
                    ...
                    htmlBody += '</table><br/><br/>Please can you extend as appropriate and notify '+bnfMap.get(cBNF.Id).Revenue_Analyst__r.User__r.Name+' ('+bnfMap.get(cBNF.Id).Revenue_Analyst__r.User__r.Email+') once complete.'+
            ...
            //BNF Metrics Processing Time Calculation
            ...
            {   
                ...
                if(cBNF.BNF_Status__c==MDM_Defines.BnfStatus_Map.get('SUBMITTED'))
                {
                    ...
                    {
                        if(Opportunity_Map.get(cBNF.opportunity__c).Actual_Close_Timestamp__c!=null)
                            cBNF.sales_start_date__c= cBNF.CreatedDate < Opportunity_Map.get(cBNF.opportunity__c).Actual_Close_Timestamp__c ? Opportunity_Map.get(cBNF.opportunity__c).Actual_Close_Timestamp__c : cBNF.CreatedDate ; 
                        else
                    ...
                    if (bnfMap.get(cBNF.id).Revenue_Analyst__r.Is_SAP_Revenue_Analyst__c == false ||
                        (bnfMap.get(cBNF.id).Revenue_Analyst__r.Is_SAP_Revenue_Analyst__c == true && 
                         bnfMap.get(cBNF.id).Revenue_Analyst__r.RA_Sales_Orgs__c!=null && 
                         !String.valueof(bnfMap.get(cBNF.id).Revenue_Analyst__r.RA_Sales_Orgs__c).contains(cBNF.IMS_Sales_org_code__c)
                        ))
                    {
                    ...
                    else
                    {
                        ...
                        if(cBNF.Time_to_1st_RA_Submission__c==null)
                        {
                            if(Opportunity_Map.get(cBNF.opportunity__c).Actual_Close_Timestamp__c!=null)
                            {
                                cBNF.Time_to_1st_RA_Submission__c=Cal_BNF_Metric_ProcessingTime.CalculateWorkingHoursBetween(cBNF.RA_First_Submission_Date__c,Opportunity_Map.get(cBNF.opportunity__c).Actual_Close_Timestamp__c,GMTTimeShift);
                            }
                            else
                ...
                if(cBNF.BNF_Status__c==MDM_Defines.BnfStatus_Map.get('LO_ACCEPTED'))
                {
                    ...
                    cBNF.LO_End_Date__c=system.now();
                    cBNF.LO_Processing_Time__c= cBNF.LO_Processing_Time__c==null ? 0.0 :  cBNF.LO_Processing_Time__c;
                    
                    //Calculate Approver timezone shift
                    TimeZone tz = TimeZone.getTimeZone(bnfMap.get(cBNF.id).Revenue_Analyst__r.User__r.TimeZoneSidKey);
                    integer GMTTimeShift=tz.getOffset(cBNF.LO_End_Date__c);
                    
                    cBNF.LO_Processing_Time__c=Cal_BNF_Metric_ProcessingTime.CalculateWorkingHoursBetween(cBNF.LO_End_Date__c,cBNF.LO_Start_Date__c,GMTTimeShift) + cBNF.LO_Processing_Time__c;
