...
export default class Lwc_csm_related_list extends LightningElement {
    @api objectName;
    @api type;
    @api iconName;
    @api title;
        ...
        return data.map(item => {
            return {
                ...
                downloadUrl: item.latestPublishedVersionId
                    ? `/support/sfc/servlet.shepherd/version/download/${item.latestPublishedVersionId}`
                    : `/support/servlet/servlet.FileDownload?file=${item.id}`
            };
        });
    }
