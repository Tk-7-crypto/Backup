...
import getArchiveSSData from '@salesforce/apex/CNT_PSA_ArchiveSsEjListView.getArchiveSSData';
...
export default class Lwc_psa_archivesearchstrategylistview extends  NavigationMixin(LightningElement) {
    @api recordId;
    ...
    @wire(getArchiveSSData, {recordId: '$recordId' })
    wireMethod({ data, error }) {
        if (data) {
            ...
            this.Archive_search_strategy = JSON.parse(JSON.stringify(data));
            if (tempData.length > 0) {
    ...
    /*onHandleClick(event){
        ...
        this[NavigationMixin.Navigate]({
            ...
            attributes: {
                objectApiName: 'Archive_Search_Strategy__c',
                actionName: 'new'
            },
            state: {
    ...
    @api viewAll(){
        ...
        window.open('/lightning/n/PSA_View_All?c__RecordId='+this.recordId+'&c__HeadingName=Archive Search Strategies&c__IconName=custom:custom68','_self');
