({
    ...
    itemChangeHelper : function(component, event, helper) {
        ...
        if(resultWrapper == null || resultWrapper.length == 0) {
            ...
            var displyToast = component.get("v.displaytoast");
            if(resultWrapper[0]["totalRecords"] > resultWrapper[0]["recordLimit"] && displyToast){
                var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
                    "type": "warning",
                    "message": 'Query returns '+resultWrapper[0]["totalRecords"] +' number of results, please include additional filter criteria. The first '+resultWrapper[0]["recordLimit"]+' records have been displayed.',
                    "duration":'5000'
                });
                toastEvent.fire();
            }
