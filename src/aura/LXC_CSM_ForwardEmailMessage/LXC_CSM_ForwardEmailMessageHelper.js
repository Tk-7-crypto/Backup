({
    ...
    getQueueRelatedEmailAddress : function(component, event){
        ...
        action.setCallback(this, function(response){
            var state = response.getState();
            if(state === "SUCCESS"){
                component.set("v.subject", "FW: "+component.get("v.emailMessageRecord").Subject);
                component.set("v.bodyText", "<br/><br/><br/>----------Forwarded Message----------<br/>"+component.get("v.emailMessageRecord").HtmlBody);
                ...
                if(fromAddresses != undefined && fromAddresses.length > 0){
                    ...
                    */
                    for(var i=0; i<options.length; i++){
                        if(options[i].label == component.get("v.fromEmailToCase")){
                            flag = false;
                            break;
                        }
                    }
                    if(flag && component.get("v.fromEmailToCase") != undefined){
                        options.push({
                            label : component.get("v.fromEmailToCase"),
                            value : component.get("v.fromEmailToCase")
                        });
                    }
                    component.set("v.fromAddresses", options);
                    var emailid;
                    if(component.get("v.fromEmailToCase") != undefined)
                        emailid = component.get("v.fromEmailToCase");
                ...
                self.loadForwardDataForAttachment(component, event);
            }
            else if(state === "ERROR"){
                var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
    ...
    loadForwardDataForAttachment : function(component, event){
        var self = this;
        var action = component.get("c.selectForwardDataForAttachment");
        action.setParams({
            "recordIdValue" : component.get("v.recordId")
        });
        action.setCallback(this, function(response){
            ...
            if(state === "SUCCESS"){
                ...
                if(contentDocumentList.length > 0){
                    ...
                    for(var i=0; i<contentDocumentList.length; i++){
                        var objectRecord = contentDocumentList[i];
                        docIdArray.push(contentDocumentList[i].Id);
                        if(contentDocumentList[i].FileExtension == 'png' || contentDocumentList[i].FileExtension == 'jpg' || contentDocumentList[i].FileExtension == 'jpeg'){
                            objectRecord.FileIcon = 'doctype:image';
                        }else if(contentDocumentList[i].FileExtension == 'txt'){
                            objectRecord.FileIcon = 'doctype:txt';
                        ...
                        objectRecord.DocumentId = contentDocumentList[i].Id;
                        objectRecord.FileName = contentDocumentList[i].Title;
                        arrayRecord.push(objectRecord);
                    }
                    
                    component.set("v.showFilePills", true);
                    component.set("v.attachmentsList", arrayRecord);
                    component.set("v.emailAttachmentsList", arrayRecord);
