({
	init : function(cmp, event, helper) {
        ...
        helper.getData(cmp);
        helper.getRecordTypeHelper(cmp);
        helper.accountNameById(cmp);
	},
    
    addEmployeeAction : function(cmp, event, helper) {
        var urlEvent = $A.get("e.force:navigateToURL");
        var accountId = cmp.get("v.recordId");
        urlEvent.setParams({
            "url": "/flow/CreateNewEmployeeRelationship?recordId=" + accountId +"&retURL=" + accountId
        });
        urlEvent.fire();
    },
    
    addIQVIAAccountTeamReport : function(cmp, event, helper) {
        var action = cmp.get("c.getGlobal_Account_Team_Roll_Up_Report");
        var returnValue = '';
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                var urlEvent = $A.get("e.force:navigateToURL");
                urlEvent.setParams({ "url": returnValue+'&fv1='+accName });
                urlEvent.fire();
            }
         });
