({
    ...
    getParams: function(component, event, helper) {
		...
		var cdaObject = component.get('v.cdaRequest');
        if(cdaObject.QuintilesIMS_Business__c != null && cdaObject.QuintilesIMS_Business__c != '' && cdaObject.QuintilesIMS_Business__c == 'Real World Solutions'){
            filter = "IQVIA_Business_Area__c in ('Research & Development Solutions (RDS) (Legacy Quintiles)','Technology & Analytics Solutions (TAS) (Legacy IMS)') AND Is_Active__c = true";
        }
        if(cdaObject.QuintilesIMS_Business__c != null && cdaObject.QuintilesIMS_Business__c != '' && (cdaObject.QuintilesIMS_Business__c == 'Research & Development Solutions (RDS) (Legacy Quintiles)' || cdaObject.QuintilesIMS_Business__c == 'Q Squared')){
            if(cdaObject.Project_Specific_Indicator__c == 'No'){
                filter += ' AND is_For_RDS_Q2_Broad__c = true';
            }
        }
        return {
            filter: filter,
    ...
    getRecordsBySearchTerm: function(component, event, helper) {
        var lookupInput = (component.find('lookupInput').getElement() != null) ? component.find('lookupInput').getElement().value : '';
        
        var searchTerm = lookupInput;
        ...
        component.set('v.searchTimeout', setTimeout($A.getCallback(function() {
            ...
            getRecordsAction.setCallback(this, function(res) {
                if (res.getState() === 'SUCCESS') {
                    ...
                    if (returnValue.isSuccess && returnValue.results.searchTerm === lookupInput) {
                        var returnedRecords = [];
    ...
    addNewRecord: function(component, event, helper) {
        ...
        if (overrideNewEvent) {
            addRecordEvent = component.getEvent('lookup_evt_addNewRecord');
        } else {
