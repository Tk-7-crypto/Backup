({
        ...
        action.setCallback(this, function (response) {
            if (response.getState() === "SUCCESS" && response.getReturnValue() != '') {
                ...
                toastEvent.setParams({
                    ...
                    "type": "Error",
                    "mode": 'dismissible',
                    "message": "There is an Error in Finding the documents "+ (errors[0] && errors[0].message) ? errors[0].message : 'Something went wrong !!'
                });
                toastEvent.fire();
    ...
    getAgreementTeam : function (component) {
        var action = component.get("c.getAgreementTeams");
        var rId = (component.get("v.agrId") != undefined ? component.get("v.agrId") : component.get("v.recordId"));
        action.setParams({
            agrId: rId
        });
        action.setCallback(this, function (response) {
            ...
            if (response.getState() === "SUCCESS") {
                ...
                if(contacts.length > 0) {
                    ...
                    if (docuSignFlag) {
                        ...
                        var toastEvent = $A.get("e.force:showToast");
                 		toastEvent.setParams({
                            ...
                            "type": "Error",
                            "mode": 'dismissible',
                            "message": 'Agreement Team(s) are not marked as DocuSign Recipient. Please update Agreement Team and mark DocuSign Recipient.'
                        });
                        toastEvent.fire();
                    ...
                    component.set("v.executionOrderCheck", false);
                    ...
                    toastEvent.setParams({
                        ...
                        "type": "Error",
                        "mode": 'dismissible',
                        "message": 'There are no valid Agreement Teams available. Please Create and Try Again'
                    });
                    toastEvent.fire();
                    component.set("v.loadScreen", false);                
                    const myTimeout = setTimeout(goToAgreement, 3000);
                ...
                toastEvent.setParams({
                    ...
                    "type": "Error",
                    "mode": 'dismissible',
                    "message": (errors[0] && errors[0].message) ? errors[0].message : 'Something went wrong !!'
                });
                toastEvent.fire();
                $A.log("Errors", response.getError());
            }
    ...
    updateDocuSignEnvelope: function (component, event, helper) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (eSResult.status == 'SUCCESS') {
                component.set("v.hasError", false);
                component.set("v.errorList", '');
                window.open('/lightning/r/IQVIA_Agreement__c/' + eSResult.batchId + '/view', '_top');
            } else {
                component.set("v.loaded", false);
                component.set("v.hasError", true);
                component.set("v.errorList", eSResult.message);
    ...
    getPreviewUrl: function (component, helper) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (response.getState() === "SUCCESS") {
                ...
                toastEvent.fire();
                $A.log("Errors", response.getError());
                console.log("Errors::" + (errors[0] && errors[0].message) ? errors[0].message : 'Something went wrong !!');
            }
