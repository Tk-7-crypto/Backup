({
    ...
    isValidDate: function(component, selectedDate) {
        var todayDate = new Date();
        todayDate = new Date((todayDate.getMonth()+1) + "/" + todayDate.getDate() + "/" + todayDate.getFullYear());
        var millisecondsPerDay = 1000 * 60 * 60 * 24;
        var millisBetween = selectedDate.getTime() - todayDate.getTime();
        var days = Math.floor(millisBetween / millisecondsPerDay);
        var notBeforeInterval = component.get('v.notBeforeInterval');
        var notBeforeIntervalType = component.get('v.notBeforeIntervalType');
        var notAfterInterval = component.get('v.notAfterInterval');
        var notAfterIntervalType = component.get('v.notAfterIntervalType');
        if(notBeforeInterval != null) {
            if(notBeforeIntervalType == 'years') {
                notBeforeInterval = notBeforeInterval * 365 * -1;
            }
            else {
                notBeforeInterval = notBeforeInterval * -1;
            }
            console.log(days + ':' + notBeforeInterval);
            if(days < notBeforeInterval) {
                return false;
            }
        }
        if(notAfterInterval != null) {
            if(notAfterIntervalType == 'years') {
            	notAfterInterval = notAfterInterval * 365;
            }
            else {
                notAfterInterval = notAfterInterval;
            }
            console.log(days + ':' + notBeforeInterval);
            if(days > notAfterInterval) {
                return false;
            }
        }
        return true;
    ...
    processDateValue: function (component) {
        ...
        if ($A.util.isEmpty(dateString)) {
            ...
            var formattedDisplayDate = $A.localizationService.formatDate(currentDate, datePattern);
